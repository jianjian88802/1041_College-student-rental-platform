<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="房产证" prop="fangchanzheng">
          <file-upload
          tip="点击上传房产证"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.fangchanzheng" label="房产证" prop="fangchanzheng">
            <el-button type="text" size="small" @click="download(ruleForm.fangchanzheng)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shenfenzheng" label="身份证" prop="shenfenzheng">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shenfenzheng.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xinxishenpi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinxishenpiCrossAddOrUpdateFlag = false;
                  this.parent.search();
